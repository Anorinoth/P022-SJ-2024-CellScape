...
#' - `packages_cran_archive.txt`: Archived CRAN packages in format "package@version"
#'
#' GitHub packages install first so dev versions are available when
#' Bioconductor packages resolve dependencies (e.g. ggtree for treekoR).
#'
...
#'
#' @export
install_packages <- function(config_dir = NULL,
                             install_bioc = TRUE,
                             install_cran = TRUE,
                             install_github = TRUE,
  ...
  if (is.null(config_dir)) {
    stop("config_dir must be supplied. Pass dirs$CONFIG_DIR from setup_project().")
  }
  ...
  # ============================================================================
  # 1. Install GitHub Packages (before Bioc — some Bioc packages depend on
  #    dev versions only available from GitHub, e.g. ggtree for treekoR)
  # ============================================================================
  ...
  # ============================================================================
  # 4. Install Archived CRAN Packages
  # ============================================================================

  archive_file <- file.path(config_dir, "packages_cran_archive.txt")
  if (file.exists(archive_file)) {
    archive_packages <- read_package_list(archive_file)

    if (length(archive_packages) > 0) {
      if (verbose) {
        message("\n4. Archived CRAN Packages")
        message("  Found ", length(archive_packages), " archived packages in config")
      }

      for (entry in archive_packages) {
        ...
        if (needs_install) {
          if (verbose) {
            message("  → Installing ", pkg_name, " ", pkg_version, " from CRAN archive...")
          }
          renv::install(entry)
          results$all_present <- FALSE
        } else {
