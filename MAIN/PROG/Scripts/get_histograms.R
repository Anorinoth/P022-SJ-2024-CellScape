...
spe <- readRDS(file.path(dirs$CHECKPOINT_DIR, "combined_spe_after_section7_v2.rds"))
df_all <- data.frame(
    id = colnames(spe),
    x = spatialCoords(spe)[, 1],
    y = spatialCoords(spe)[, 2],
    imageID = colData(spe)$imageID,
    domain = colData(spe)$annotation,
...
# Output dir for histogram plots
plots_dir <- file.path(dirs$RES_DIR, "SpatialDomains", "Plots")
...
ggsave(file.path(plots_dir, "Patch_Areas_Histogram.png"), p1, width = 10, height = 8)
...
ggsave(file.path(plots_dir, "Boundary_Lengths_Histogram.png"), p2, width = 10, height = 8)
